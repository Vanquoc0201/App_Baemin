...
export class AuthService {
    ...
    async register(body: RegisterDto) {
        try {
            ...
            if (userExists) throw new BadRequestException('Email đã tồn tại vui lòng đăng nhập');
    ...
    async login(body: LoginDto) {
      ...
      const isPassword = await bcrypt.compare(body.password, userExists.password);
      if (!isPassword) {
